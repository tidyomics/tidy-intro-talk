...
## Bootstrapping and matching

We developed a package
[nullranges](https://nullranges.github.io/nullranges), as a modular tool
to assist with making genomic comparisons. It doesn't do enrichment
analysis but provides null genomic range sets for investigating various
hypotheses.
...
```{r}
#| echo: false
counts <- matrix(rpois(1e5,100),ncol=100)
rowdata <- DataFrame(x=1:1e3)
...
tidy_version <- function(x) {
  suppressMessages({
    x |> group_by(.feature) |> summarize(ave_count = mean(counts))
  })
}
```
...
-   main contributions to variance (e.g. PCA, see `plotPCA` for bulk and
    [OSCA](https://bioconductor.org/books/release/OSCA/) for sc)
-   column and row densities (`tidySE` allows directly plotting `geom_density`
    of rows/columns, or `geom_violin`)
